...
Filled in from Will's own real testing session against the live deployed app (not written preemptively by the
build process — see the git history for the difference between what was found/fixed during building versus what
came from actually using it).
...
- **Small upload file-size limit, requiring compressed files.** Confirmed real and unavoidable at MVP scope —
  ...
  entirely) if the product gets real traction past MVP — not needed for tomorrow.
- **"Both loading screens broke" initially.** Best interpretation, not confirmed with Will directly: this most
  likely lines up with two issues already found and fixed earlier in the build (both documented in `BUILD_LOG.md`)
  — a knowledge base's page 404ing right after creation (the SQLite-on-Vercel persistence bug, fixed by moving to a
  real Postgres database), and file uploads 500ing before the parser lazy-import fix. If this refers to something
  else, it needs a follow-up description to chase down properly rather than being guessed at further here.
- **Switching tabs mid-upload or mid-generation loses all progress.** Real, reproducible, not yet fixed: the
  ...
  **Not fixed yet** — flagged by Will as "would be cool to fix," not a submission blocker. The fix is
  straightforward (keep all three panels mounted always, toggle visibility with CSS instead of conditional
  rendering, so their state survives a tab switch) and is a good candidate for the next work session.
...
- **Grounded Q&A + practice test together, explicitly**: "having the practice test alongside an AI-powered search
  is extremely useful to explain answers." Feature request that follows directly from this: a question box
...
Will's answer to "would you use this over manually searching your notes?": **"Absolutely."** That's one real,
positive data point toward `HYPOTHESIS.md`'s decision rule — genuine directional evidence, not yet the sample size
the 70%-threshold rule was written for. `/admin` keeps accumulating real usage automatically as more people (a full
class) actually use it; the rule should be evaluated against that larger sample, not this one session alone.

## Ship decision

Will's own call: **"For an MVP I think it's great thus far."** Nothing here blocks submission — the one confirmed
open item (tab-switch state loss) and the two feature ideas (search-in-test-panel, and eventually loosening the
upload size limit) are explicitly follow-up work, not fixes required before tomorrow.
